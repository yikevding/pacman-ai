...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            # if ghosts got caught, put it in prison
            gotCaught=noisyDistance is None
            if gotCaught:
                allPossible[p]=0
                jailSpot=self.getJailPosition()
                allPossible[jailSpot]=1

            # if not caught read its evidence probability
            else:
                evidence=emissionModel[trueDistance]
                if 0<evidence<1:
                    allPossible[p]=evidence*self.beliefs[p] # from observation formula in lectue slides
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            newState=self.setGhostPosition(gameState,pos)
            newDist=self.getPositionDistribution(newState)
            for newPos, prob in newDist.items():
                allPossible[newPos]+=prob*self.beliefs[pos] # from time step formula in lecture slides
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while count!=totalParticles:
            if index<size:
                particle=self.legalPositions[index]
                self.particles.append(particle)
                index+=1
                count+=1
            # if surpass size, reset index to the first cell
            else:
                index=0
    ...
    def observe(self, observation, gameState):
        ...
        # check if ghost got caught
        gotCaught=noisyDistance is None

        if gotCaught:
            jailSpot=self.getJailPosition()
            for i in range(0,totalParticles):
                self.particles[i]=jailSpot
            return
        ...
        # if all weights are zero, then initialize it uniformly
        sumWeights=weights.totalCount()
        if sumWeights==0:
            self.initializeUniformly(gameState)
            return

        # otherwise generate next sample
        weights.normalize()
        self.particles=[]
        for i in range(0,totalParticles):
            newPos=util.sample(weights)
    ...
    def elapseTime(self, gameState):
        ...
        belief distribution
        """
        currentParticles=self.particles
        newParticles=[]

        totalParticles=self.numParticles
        for i in range(0,totalParticles):
            ghostPos=currentParticles[i]
            newState=self.setGhostPosition(gameState,ghostPos)
            newDist=self.getPositionDistribution(newState)
            newPos=util.sample(newDist)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        totalParticles=self.numParticles
        # use cartesian product to get all combination of ghost positions
        cartesian=itertools.product(self.legalPositions,repeat=totalGhosts)
        allPossible=list(cartesian)
        size=len(allPossible)

        # shuffle it three times to ensure even placement
        # I choose 3 times randomly (1, 2 also work)
        for i in range(0,2):
            random.shuffle(allPossible)

        # count= number of particles been put
        # index= position of the grid current particle will be put
        self.particles=[]
        index=0
        count=0
        ...
        while count!=totalParticles:
            if index<size:
                particle=allPossible[index]
                self.particles.append(particle)
                index+=1
                count+=1
            # if surpass size, then reset it to 0 to put it in the first cell
            else:
                index=0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # check if a ghost is eaten; if not update its weight
        for particleIndex in range(0,totalParticles):
            currentParticle=self.particles[particleIndex]
            ghostWeight=1
            for ghostIndex in range(0,totalGhosts):
                gotCaught=noisyDistances[ghostIndex] is None
                currentGhost=currentParticle[ghostIndex]
                currentEvidence=emissionModels[ghostIndex]

                if gotCaught:
                    updatedParticle=self.getParticleWithGhostInJail(currentParticle,ghostIndex)
                    self.particles[particleIndex]=updatedParticle
                    continue
                if not gotCaught:
                    trueDistance=util.manhattanDistance(pacmanPosition,currentGhost)
                    ghostWeight*=currentEvidence[trueDistance] ## joint distribution, prob of position of all ghosts
            weights[self.particles[particleIndex]]+=ghostWeight
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            totalGhosts=self.numGhosts
            for i in range(0,totalGhosts):
                newState=setGhostPositions(gameState,oldParticle)
                ghost=self.ghostAgents[i]
                newDist=getPositionDistributionForGhost(newState,i,ghost)
                newPos=util.sample(newDist)
