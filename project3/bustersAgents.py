...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            likelyPos.append(ghost.argMax())

        # among all these ghosts, find the nearest one
        min=float("inf")
        ghostPos=None
        ...
        # choose next action that move pacman closest to the nearest ghost
        min=float("inf")
        optimalAction=None
        for action in legal:
            nextPos=Actions.getSuccessor(pacmanPosition,action)
            distance=self.distancer.getDistance(nextPos,ghostPos)
            if distance<min:
                min=distance
